...
export default function EditProductScreen() {
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      try {
        ...
        setColorVariants((product.colorVariants || []).map((v: any) => ({ name: v.name || "", imageUrl: v.media?.[0]?.url || "", imageUri: "", sizes: v.sizes || product.availableSizes || [...PRODUCT_SIZES], stockBySize: Object.fromEntries((v.sizes || product.availableSizes || [...PRODUCT_SIZES]).map((size: string) => [size, String(v.stockBySize?.[size] ?? 0)])) })));
        setDrop(product.drop || { active: false, text: "", startsAt: "", endsAt: "" });
        setColorVariants((product.colorVariants || []).map((v: any) => ({ name: v.name || "", imageUrl: v.media?.[0]?.url || "", imageUri: "", sizes: v.sizes || product.availableSizes || [...PRODUCT_SIZES], stockBySize: Object.fromEntries((v.sizes || product.availableSizes || [...PRODUCT_SIZES]).map((size: string) => [size, String(v.stockBySize?.[size] ?? 0)])) })));
  ...
  const save = async (statusOverride?: "DRAFT" | "PUBLISHED") => {
    ...
    try {
      ...
      await apiPut(`/products/${id}`, { title: form.title.trim(), sku: form.sku.trim(), price: usd, regionalPrices: { USD: usd, PKR: pkr, INR: inr }, sizes: selectedSizes, media: uploaded, imageUrl: uploaded[0]?.url, description: form.description.trim(), category: form.category.trim(), stockQuantity: Math.floor(stockQuantity), status: savedStatus, drop, colorVariants: savedColorVariants, });
