...
export default function EditProductScreen() {
  ...
  const [measurementsBySize, setMeasurementsBySize] = useState<Record<string, { chest: string; length: string; shoulder: string }>>({});
  ...
  useEffect(() => {
    const loadProduct = async () => {
      ...
      try {
        ...
        setStockBySize(Object.fromEntries((Array.isArray(product.availableSizes) && product.availableSizes.length ? product.availableSizes : [...PRODUCT_SIZES]).map((size: string) => [size, String(product.stockBySize?.[size] ?? "")])))
        setMeasurementsBySize(Object.fromEntries((Array.isArray(product.availableSizes) && product.availableSizes.length ? product.availableSizes : [...PRODUCT_SIZES]).map((size: string) => [size, { chest: String(product.measurementsBySize?.[size]?.chest || ""), length: String(product.measurementsBySize?.[size]?.length || ""), shoulder: String(product.measurementsBySize?.[size]?.shoulder || "") }])));
        setDrop(product.drop || { active: false, text: "", startsAt: "", endsAt: "" });
        setColorVariants((product.colorVariants || []).map((v: any) => ({ name: v.name || "", imageUrl: v.media?.[0]?.url || "", imageUri: "", colorMedia: (v.media || []).map((asset: any, assetIndex: number) => ({ ...asset, id: `${asset.url}-${assetIndex}` })), sizes: v.sizes || product.availableSizes || [...PRODUCT_SIZES], stockBySize: Object.fromEntries((v.sizes || product.availableSizes || [...PRODUCT_SIZES]).map((size: string) => [size, String(v.stockBySize?.[size] ?? 0)])) })));
