...
export default function EditProductScreen() {
  ...
  const save = async (statusOverride?: "DRAFT" | "PUBLISHED") => {
    ...
    try {
      ...
      await apiPut(`/products/${id}`, { title: form.title.trim(), sku: form.sku.trim(), price: usd, regionalPrices: { USD: usd, PKR: pkr, INR: inr }, sizes: selectedSizes, media: uploaded, imageUrl: uploaded[0]?.url, description: form.description.trim(), category: form.category.trim(), stockQuantity: Math.floor(stockQuantity), status: savedStatus });
