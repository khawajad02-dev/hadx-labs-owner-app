  ...
  const handleSave = async (status: "DRAFT" | "PUBLISHED") => {
    ...
    try {
      ...
      await apiPost("/products", {
        ...
        status,
        drop, colorVariants: colorVariants.map((variant) => ({ ...variant, media: variant.imageUrl ? [{ url: variant.imageUrl, type: "image" }] : [], stockBySize: Object.fromEntries(Object.entries(variant.stockBySize).map(([size, value]) => [size, Number(value) || 0])) })),
      });
