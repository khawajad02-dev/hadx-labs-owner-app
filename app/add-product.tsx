...
export default function AddProductScreen() {
  ...
  const [colorVariants, setColorVariants] = useState<Array<{ name: string; imageUrl: string; imageUri?: string; sizes: string[]; stockBySize: Record<string, string> }>>([]);
  ...
  const pickColorImage = async (index: number) => {
    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) { Alert.alert("Gallery access needed", "Allow gallery access to choose a color image."); return; }
    ...
    setColorVariants((all) => all.map((variant, variantIndex) => variantIndex === index ? { ...variant, imageUri: asset.uri, imageUrl: asset.uri } : variant));
  };
  ...
  const handleSave = async (status: "DRAFT" | "PUBLISHED") => {
    ...
    try {
      ...
      for (const variant of colorVariants) {
        ...
        if (variant.imageUri) { const uploaded = await uploadPickedMedia({ uri: variant.imageUri, type: "image", mimeType: "image/jpeg", fileName: `${variant.name || "color"}.jpg` }); imageUrl = uploaded.publicUrl; }
        savedColorVariants.push({ name: variant.name.trim(), media: imageUrl ? [{ url: imageUrl, type: "image" as const }] : [], sizes: variant.sizes, stockBySize: Object.fromEntries(Object.entries(variant.stockBySize).map(([size, value]) => [size, Number(value) || 0])) });
      }
